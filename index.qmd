...
This model yielded the following regression equation for a Manual, Diesel vehicle sold by an individual:
...
Visually the results are summarized in Figure xx, and we see slight differences in xyz.
 ...
 below are the coefficients of the best performing model Hold Out
\begin{align*}Selling\_Price = -6.019555e^{02} -3.669399e^{-06}Kms\_Driven +6.441331Fuel\_TypeDiesel \\ 
+1.825635Fuel\_TypePetrol-4.185450Seller\_TypeIndividual\\
-3.891009TransmissionManual+ 3.023458e^{-01}Year 
\end{align*}

When looking results of the linear model, we can see that just based on high $R^2$ values, and low RMSE and MAE, the hold out method would look to be the superior technique for cross-validation, which seems to disagree with our literature review. However, with a 70/30 data split, or a 211/90 train/test split, it seems to be random chance to have the high $R^2$. This is confirmed by reviewing the various predictor evaluations for the k-folds and we can see there is a range of RMSE, MAE and $R^2$ values.
...
again the hold out method performed the best but not by a large margin. Below is the model. 

\begin{align*}Selling\_Price =  -1.879796e^{02} -1.097771e^{-06}Kms\_Driven 9.574837e^(-1)Fuel\_TypeDiesel \\ 
+ 4.335012e^{-1}Fuel\_TypePetrol-2.078542Seller\_TypeIndividual\\
-3.952547e^{-01}TransmissionManual+ 9.409054e^{-02}Year 
\end{align*}


We see that the hold-out method looks to be performing the worst. However, the other cross-validation techniques yield consistent results, with the k-folds methods being good techniques for modeling predictions.

### Conlusion

This paper shows various cross-validation techniques that can be used for modeling validation, including hold out, k and repeated k-folds, leave-one-out and Monte Carlo cross-validation.  As cross-validation tends to be used in machine learning on large data, or in situations where it may be hard to gather large samples, our example allowed us to review the full data set's performance with linear regression and then compare the performance of the cross validation techniques. We compared both a linear model and a transformed linear model and found that for the most part many of these cross validation techniques performed about the same and were able to fit accurate models. While k-folds and repeated k-folds seemed to perform the best, most of these techniques seem sufficient to help validate a model based on this example. 
