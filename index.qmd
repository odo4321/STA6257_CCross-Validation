...
RMSE= √(Σ (Pi – Oi)2 / n), with Pi being the predicted value for the ith observation, Oi being the ith observed value, and n being the sample size. MAE describes the typical magnitude of the residuals and is calculated by:
(1/n)Σ|y-Y|, where y is the actual output value and Y is the predicted output. R2 describes how well the model predictors explains the response variable variation. Better models tend to have low RMSE, MAE, and higher R2 values.
